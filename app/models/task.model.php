...
class TaskModel{
    ...
    public function getProducts($params = NULL){
        ...
        if(isset($_GET['page'])){
            ...
            if(isset($_GET['id_categoria']) && isset($_GET['order'])) {
                $query = $db->prepare("SELECT * FROM `producto` WHERE id_categoria = $_GET[id_categoria] ORDER BY id $_GET[order] LIMIT 3 OFFSET $pag ");
            }
            elseif(isset($_GET['id_categoria'])) {
                $query = $db->prepare("SELECT * FROM `producto` WHERE id_categoria = $_GET[id_categoria] LIMIT 3 OFFSET $pag ");
            }
            elseif(isset($_GET['order'])) {
                $query = $db->prepare("SELECT * FROM `producto` ORDER BY id $_GET[order] LIMIT 3 OFFSET $pag ");
            }
        }elseif (isset($_GET['id_categoria']) && isset($_GET['order'])){
            $query = $db->prepare("SELECT * FROM `producto` WHERE id_categoria = $_GET[id_categoria] ORDER BY id $_GET[order] ");
        }elseif (isset($_GET['id_categoria'])){
            $query = $db->prepare("SELECT * FROM `producto` WHERE id_categoria = $_GET[id_categoria] ");
        }elseif (isset($_GET['order'])){
            $query = $db->prepare("SELECT * FROM `producto` ORDER BY id $_GET[order] ");
        }
